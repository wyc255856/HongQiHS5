...
#import "HS5JKEventHandler.h"
#import "JKEventHandler+HS5Car.h"
#import "HS5ShareManager.h"
#import "HS5LoadFailedAlertView.h"
#import "HS5LoadingProgressView.h"
#import "HS5CAR_SSZipArchive.h"
#import "HS5CAR_AFHTTPRequestOperation.h"




@interface HS5JKWKWebViewController ()<WKNavigationDelegate, WKUIDelegate, LoadFailedAlertViewDelegate>
...
@implementation HS5JKWKWebViewController
...
- (void)viewDidLoad {
    ...
    [HS5ShareManager shareInstance].wkWebVC = self;
...
- (void)configureWKWebview{
    ...
    WKUserScript *usrScript = [[WKUserScript alloc] initWithSource:[HS5JKEventHandler shareInstance].handlerJS injectionTime:WKUserScriptInjectionTimeAtDocumentEnd forMainFrameOnly:YES];
    ...
    [config.userContentController addScriptMessageHandler:[HS5JKEventHandler shareInstance] name:EventHandler];
    ...
    NSString *sUpLoad =[userDefaults objectForKey: @"haveLocalHS5Packaged"] ;
    ...
    if([userDefaults objectForKey:@"webviewHS5LoadMode"]){
        ...
        [self.webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/?upLoad=%@",[HS5ShareManager shareInstance].wkWebVC.url,sUpLoad]]]];
    }
    [self.view addSubview:_webView];
    
    [HS5JKEventHandler getInject:_webView];
...
- (void)showLoadFailedAlert{
    HS5LoadFailedAlertView *alertView = [[HS5LoadFailedAlertView alloc]initWithTitle:@"" message:@"" delegate:self leftButtonTitle:@"确定" rightButtonTitle:@"取消"];
    [alertView show];
}
...
// 输入框弹窗的button点击时回调
- (void)declareAbnormalAlertView:(HS5LoadFailedAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
...
- (void)releaseZipFilesWithUnzipFileAtPath:(NSString *)zipPath Destination:(NSString *)unzipPath{
    NSError *error;
    if ([HS5CAR_SSZipArchive unzipFileAtPath:zipPath toDestination:unzipPath overwrite:YES password:nil error:&error delegate:self]) {
//        NSLog(@"success");
//        NSLog(@"unzipPath = %@",unzipPath);
        ...
        if([userDefaults objectForKey:@"webviewHS5LoadMode"]){
            if(nWebViewLoadMode){
                [_webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/?upLoad=%@",_url,[userDefaults objectForKey:@"upHS5Load"]]]]];
            }else{
                NSString *strLocalCarName = [userDefaults objectForKey:@"localHS5CarModelName"];
                NSURL *temDirURL = [[NSURL fileURLWithPath:NSTemporaryDirectory()] URLByAppendingPathComponent:[NSString stringWithFormat:@"%@/index.html",strLocalCarName]];
                [_webView loadFileURL:temDirURL allowingReadAccessToURL:temDirURL];
...
//    NSLog(@"filePath: %@", filePath);
    ...
    HS5LoadingProgressView *alertView = [[HS5LoadingProgressView alloc] initProgressView];
    [alertView show];
...
//        NSLog(@"%f",progress);
    ...
    //成功和失败回调
    [operation setCompletionBlockWithSuccess:^(HS5CAR_AFHTTPRequestOperation *operation, id responseObject) {
        
        // 下载完成保存状态、现在车型名称
        [userDefaults setObject:[NSNumber numberWithInteger: STATE_HAVE_LOAD] forKey:@"haveLocalHS5Packaged"];
        [userDefaults setObject:strCarName forKey:@"localHS5CarModelName"];
        
        //保存本地资源版本号
        [userDefaults setObject:[userDefaults objectForKey:@"HS5newVersion"] forKey:@"HS5LocalVersion"];
        
        //设置模型
        [userDefaults setInteger:0 forKey:@"webviewHS5LoadMode"];
    ...
    } failure:^(HS5CAR_AFHTTPRequestOperation *operation, NSError *error) {
