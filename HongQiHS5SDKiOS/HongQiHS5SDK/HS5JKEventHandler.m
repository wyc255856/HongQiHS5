...
@interface HS5JKEventHandlerEmptyObject :NSObject

@end

@implementation HS5JKEventHandlerEmptyObject

@end

@implementation HS5JKEventHandler

static HS5JKEventHandler * _handler= nil;
+ (instancetype)shareInstance{
    
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _handler =[HS5JKEventHandler new];
...
- (NSString *)getJsString{
    ...
    NSString *path =[resourceBundle pathForResource:@"JKEventHandler" ofType:@"js"];
...
+ (void)getInject:(WKWebView *)webView{
    ...
    [HS5JKEventHandler shareInstance].webView = webView;
...
- (void)newInteractWitMethodName:(NSString *)methodName params:(NSDictionary *)params success:(void(^)(id response))successCallBack failure:(void(^)(id response))failureCallBack{
    if (params) {
        ...
        if (successCallBack) {
            ...
            successBlock = [HS5JKEventHandlerEmptyObject class];
        }
        ...
        if (failureCallBack) {
            ...
            failureBlock = [HS5JKEventHandlerEmptyObject class];
        }
...
- (id)_jkPerformSelector:(SEL)aSelector withObjects:(NSArray *)objects {
    ...
    for (id object in objects) {
        ...
        if (![tempObject isKindOfClass:[NSObject class]]) {
            if ([tempObject isSubclassOfClass:[HS5JKEventHandlerEmptyObject class]]) {
                tempObject = nil;
            }
        }
