...
@interface HS5WelcomeViewController (){
    int _timeCount;
    BOOL Finish;
    BOOL Splash;
    BOOL HS5Visitor;
...
@implementation HS5WelcomeViewController
...
//            }
//            [userDefaults setObject:sModolName  forKey:@"chooseHS5CarModelName"];
//            HS5Visitor = NO;
//            [userDefaults setObject:@"NO" forKey:@"HS5Visitor"];
//
//        }else{
//            [userDefaults setObject:@"YES" forKey:@"HS5Visitor"];
//            HS5Visitor = YES;
//        }
//
...
//                sModolName = typeManualComfortable;
//            }
//            [userDefaults setObject:carName  forKey:@"chooseHS5CarModelName"];
//        }
        if(![[HS5ShareManager shareInstance] isBlankString:carName]){
            NSString *sModolName = kFuncGetCarTypeByCarName(carName);
            if(!sModolName){
                sModolName = typeManualComfortable;
            }
...
- (void)goNext {
    ...
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    // 是否选择车型
    //NSInteger chooseCarModelIndex = [[userDefaults objectForKey: @"chooseHS5CarModelIndex"] integerValue];
    NSString *strCarName = [userDefaults objectForKey:@"chooseHS5CarModelName"];
    // 下载本地资源包 版本号字段 （默认本地资源版本号为1.0.0）
    if(![userDefaults objectForKey:@"HS5LocalVersion"]){
        [userDefaults setObject:@"1.0.0" forKey:@"HS5LocalVersion"];
        // 是否需要下载字段（1：需要 0:不需要 为了拼接后面的设置页面的链接 实现下载最新资源包）
        [userDefaults setObject:@"1" forKey:@"upHS5Load"];
    }
    
    //获取服务器最新资源版本号
    [self getAppNewVersion];

    if(![[HS5ShareManager shareInstance] isBlankString:strCarName]){
        ...
        HS5ChooseCarModelViewController *vc = [[HS5ChooseCarModelViewController alloc] init];
